...
async function getOverallRatingByHotelId(request, response) {
  const { hotelId } = request.params;
  try {
    const overallRating = await Review.aggregate([
...
async function getLikeUsers(request, response) {
  ...
  try {
    const { likes } = await Review.findOne({ _id: reviewId }).populate({
      path: 'likes',
      select: ['_id', 'email', 'displayName', 'role'],
    });
...
async function getDislikeUsers(request, response) {
  ...
  try {
    const { dislikes } = await Review.findOne({ _id: reviewId }).populate({
      path: 'dislikes',
      select: ['_id', 'email', 'displayName', 'role'],
    });
